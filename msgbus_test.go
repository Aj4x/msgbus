...
func TestSubscribe(t *testing.T) {
	t.Run("Valid subscription", func(t *testing.T) {
		...
		handler := make(MessageHandler[[]byte], 10)
	...
	t.Run("Nil handler", func(t *testing.T) {
		...
		if !errors.Is(err, ErrNilSubChannel) {
	...
	t.Run("Multiple subscriptions to same topic", func(t *testing.T) {
		bus := NewMessageBus[[]byte]()
		topic := Topic("test-topic")
		handler1 := make(MessageHandler[[]byte], 10)
		handler2 := make(MessageHandler[[]byte], 10)
...
func TestConcurrentAccess(t *testing.T) {
	t.Run("Concurrent subscriptions and publications", func(t *testing.T) {
		...
		handlers := make([]MessageHandler[[]byte], topicCount)

		for i := 0; i < topicCount; i++ {
			topics[i] = Topic(fmt.Sprintf("topic-%d", i))
			handlers[i] = make(MessageHandler[[]byte], pubCount)
...
func TestErrorCases(t *testing.T) {
	t.Run("Error type implements error interface", func(t *testing.T) {
		var err error = ErrNilSubChannel
...
func TestTimeout(t *testing.T) {
	t.Run("Publish with slow consumer", func(t *testing.T) {
		...
		handler := make(MessageHandler[[]byte])
